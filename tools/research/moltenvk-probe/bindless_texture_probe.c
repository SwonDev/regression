...
int main(int argc, char** argv) {
    ...
    const char* spv = "probe-bindless-tex.spv";
    ...
    printf("caso bindless de TEXTURAS: %u escritas, el shader muestrea la %u\n", reservados, indice);
    ...
    VkDescriptorSetLayoutBinding b1[2]={
        { .binding=0,.descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,.descriptorCount=RECURSOS_DECLARADOS,.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT },
        { .binding=1,.descriptorType=VK_DESCRIPTOR_TYPE_SAMPLER,.descriptorCount=1,.stageFlags=VK_SHADER_STAGE_COMPUTE_BIT } };
    // La cuenta variable debe ir en la ÚLTIMA binding; se ordena binding 1 primero en las flags.
    VkDescriptorBindingFlags bf[2]={ VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT, 0 };
    VkDescriptorSetLayoutBindingFlagsCreateInfo bfi={ .sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO,
        .bindingCount=2,.pBindingFlags=bf };
    VkDescriptorSetLayoutCreateInfo l1={ .sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,.pNext=&bfi,.bindingCount=2,.pBindings=b1 };
    ...
    VkDescriptorPoolSize ps[3]={ { .type=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,.descriptorCount=4 },
                                 { .type=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,.descriptorCount=RECURSOS_DECLARADOS },
                                 { .type=VK_DESCRIPTOR_TYPE_SAMPLER,.descriptorCount=4 } };
    VkDescriptorPoolCreateInfo dpi={ .sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,.maxSets=2,.poolSizeCount=3,.pPoolSizes=ps };
    ...
    VkDescriptorSetAllocateInfo a1={ .sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,.descriptorPool=dp,.descriptorSetCount=1,.pSetLayouts=&dsl1 };
    ...
    // Una textura 3D real y un sampler; sólo se escriben `reservados` entradas del array.
    VkImageCreateInfo ici3={ .sType=VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,.imageType=VK_IMAGE_TYPE_3D,
        .format=VK_FORMAT_R8G8B8A8_UNORM,.extent={4,4,4},.mipLevels=1,.arrayLayers=1,
        .samples=VK_SAMPLE_COUNT_1_BIT,.tiling=VK_IMAGE_TILING_OPTIMAL,
        .usage=VK_IMAGE_USAGE_SAMPLED_BIT,.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED };
    VkImage img; CHK(vkCreateImage(dev,&ici3,NULL,&img));
    ...
    VkMemoryAllocateInfo imai={ .sType=VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,.allocationSize=imr.size,
        .memoryTypeIndex=tipoMemoria(pd,imr.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) };
    VkDeviceMemory imem; CHK(vkAllocateMemory(dev,&imai,NULL,&imem)); CHK(vkBindImageMemory(dev,img,imem,0));
    VkImageViewCreateInfo ivi={ .sType=VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,.image=img,.viewType=VK_IMAGE_VIEW_TYPE_3D,
        .format=VK_FORMAT_R8G8B8A8_UNORM,.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1} };
    VkImageView iv; CHK(vkCreateImageView(dev,&ivi,NULL,&iv));
    VkSamplerCreateInfo sci={ .sType=VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO };
    VkSampler samp; CHK(vkCreateSampler(dev,&sci,NULL,&samp));

    if (reservados > 0) {
        VkDescriptorImageInfo* diis=calloc(reservados,sizeof(VkDescriptorImageInfo));
        for(uint32_t i=0;i<reservados;i++){ diis[i].imageView=iv; diis[i].imageLayout=VK_IMAGE_LAYOUT_GENERAL; }
        VkWriteDescriptorSet w1={ .sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,.dstSet=ds1,.dstBinding=0,.dstArrayElement=0,
            .descriptorCount=reservados,.descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,.pImageInfo=diis };
        vkUpdateDescriptorSets(dev,1,&w1,0,NULL);
    }
    VkDescriptorImageInfo sdi={ .sampler=samp };
    VkWriteDescriptorSet ws={ .sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,.dstSet=ds1,.dstBinding=1,.dstArrayElement=0,
        .descriptorCount=1,.descriptorType=VK_DESCRIPTOR_TYPE_SAMPLER,.pImageInfo=&sdi };
