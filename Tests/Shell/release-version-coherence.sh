...
assert_contract() {
    ...
    [[ "$(contract_value "$installer" VERSION)" == "1.12.2" ]] || return 1
    [[ "$(contract_value "$installer" BUILD_NUMBER)" == "40" ]] || return 1
    grep -Fx 'VERSION="${REGRESSION_RELEASE_VERSION:-1.12.2}"' \
        "$release_packager" >/dev/null || return 1
    grep -Fx 'BUILD_NUMBER="${REGRESSION_RELEASE_BUILD_NUMBER:-40}"' \
        "$release_packager" >/dev/null || return 1
    [[ "$(contract_value "$native_packager" VERSION)" == "1.12.2" ]] || return 1
    [[ "$(contract_value "$native_packager" BUILD_NUMBER)" == "40" ]] || return 1
}

assert_contract "$INSTALLER" "$RELEASE_PACKAGER" "$NATIVE_PACKAGER" || {
    printf 'FAIL: instalador y empaquetadores no comparten el contrato 1.12.2 (40).\n' >&2
    exit 1
}
...
/usr/bin/sed -i '' 's/^VERSION="1.12.2"$/VERSION="1.11.0"/' \
...
printf 'PASS: instalador y empaquetadores comparten 1.12.2 (40) y el drift 1.11 se rechaza.\n'
